...
@Component({
    selector: 'app-occupation-group-list',
    templateUrl: './occupation-group-list.component.html',
    styleUrls: ['./occupation-group-list.component.scss']
})
export class OccupationGroupListComponent implements DoCheck {
    initForm: boolean = false;
    lang: string;

    mode: MODE_PAGE = 'LIST';

    criteria: MasOccupationGroupData = {
        occupationGroupNameEn: null,
        occupationGroupNameTh: null
    };

    items: MasOccupationGroupData[] = [];
    totalRecords: number = 0;
    rows: number = 5;

    editData: MasOccupationGroupData;
    ...
    onSearch(event?: TablePageEvent) {
        ...
        this.masterService.findOccupationGroup(this.criteria).subscribe((result) => {
    ...
    openPage(page: MODE_PAGE, id?: number) {
        if (page == 'CREATE') {
            this.editData = {
                ...
                occupationGroupCode: '',
                occupationGroupNameEn: '',
                occupationGroupNameTh: '',
                activeFlag: true
            };
            this.mode = page;
        } else if (page == 'LIST') {
            ...
            this.loaderService.start();
            this.masterService.getOccupationGroup(id).subscribe((result) => {
                this.loaderService.stop();
                if (result.status === 200) {
