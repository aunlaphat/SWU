...
@Component({
    selector: 'app-expected-learning-outcomes',
    templateUrl: './expected-learning-outcomes.component.html',
    styleUrls: ['./expected-learning-outcomes.component.scss']
})
export class ExpectedLearningOutcomesComponent implements OnInit,DoCheck {
    ...
    openPage(page: MODE_PAGE, id?: number) {
        ...
        if (page == 'CREATE') {
            ...
            this.loaderService.start();
            this.courseManagementService.getCourseSclo(id).subscribe((result) => {
                this.loaderService.stop();
    ...
    deleteCourseSclo(id: number) {
        this.confirmationService.confirm({
            key: 'confirm1',
            icon: 'pi pi-exclamation-triangle',
            accept: () => {
                this.loaderService.start();
                this.courseManagementService.deleteCourseSclo(id).subscribe(({ status, message }) => {
                    this.loaderService.stop();
                    this.initForm = false;
                    if (status === 200) {
                        this.messageService.add({
