...
import { CoursepublicAttachData } from 'src/app/models/course-management';
import { CourseManagementService } from 'src/app/services/course-management.service';
import { UploadFileService } from 'src/app/services/upload-file.service';

@Component({
    selector: 'app-round-teaching-supporting-documents',
    templateUrl: './round-teaching-supporting-documents.component.html',
    styleUrls: ['./round-teaching-supporting-documents.component.scss']
})
export class RoundTeachingSupportingDocumentsComponent {
    showError: boolean = false;

    @Input() item!: CoursepublicAttachData;
    ...
    onSave() {
        ...
        if (!!!this.item.fileNameTh ||
            !!!this.item.fileNameEn ||
            !!!this.item.filename) {
            this.showError = true;
        ...
        if (this.mode === 'CREATE') {
            this.courseManagementService.postCoursepublicAttach(this.item).subscribe((result) => {
                this.loaderService.stop();
            ...
            this.courseManagementService
                .putCoursepublicAttach(this.item.coursepublicAttachId, this.item)
                .subscribe((result) => {
                    this.loaderService.stop();
                    if (result.status === 200) {
                        this.messageService.add({
