...
@Slf4j
public class EnrollmentListExportProvider extends InterfaceNativeGenerateRowData<ReportEnrollmentListData>{
	@NonNull
    ...
    private List<ReportEnrollmentListData> rawData;
    private ReportEnrollmentListData Total;

    @Override
    public DataResponse<ReportEnrollmentListData> getData() throws Exception {
        ...
        return DataResponse.<ReportEnrollmentListData>builder()
//            .datas(rawData)
			...
			public void writeBody(DataResponse data, SXSSFWorkbook workbook, Sheet sheet, AtomicInteger rowIndex) {
				...
				for (Map<String, Object> dataReport  : datas) {
					...
					wt.stringWrite((String) dataReport.get("full_course_th"), columnIndex.getAndIncrement())
					  .stringWrite((String) dataReport.get("full_name_th"), columnIndex.getAndIncrement())
				      .stringWrite((String) dataReport.get("receipt_no"), columnIndex.getAndIncrement())
                      .dateWrite((Date) dataReport.get("receipt_date"), columnIndex.getAndIncrement()) //receipt_date
                      .stringWrite((String) dataReport.get("pay_type_th"), columnIndex.getAndIncrement()) 
                      .bigDecimalWrite((BigDecimal) dataReport.get("payment_amount"), columnIndex.getAndIncrement()) 
                      .dateWrite((Date) dataReport.get("transaction_datetime"), columnIndex.getAndIncrement()) //transaction_datetime
					  .stringWrite((String) dataReport.get("study_status_th"), columnIndex.getAndIncrement());
    ...
    public HeaderHandler writeHeader() {
        return new HeaderHandler() {
            ...
            public void writeHeader(SXSSFRow headerRow, SXSSFWorkbook workbook) {
                ...
                // List<String> headers = new ArrayList<String>(Arrays.asList("No", "Course", "Student", 
                // 		"Receipt number", "Receipt date", "Payment type", "Registration fee", "Payment date", "Status"));
                
                List<String> headers = new ArrayList<String>(Arrays.asList("ลำดับ", "คอร์ส", "ผู้เรียน", 
                		"เลขที่ใบเสร็จ", "วันที่ออกใบเสร็จ", "ประเภทการชำระเงิน", "ค่าลงทะเบียน", "วันที่ชำระเงิน", "สถานะ"));
