...
@Slf4j
public class CanceledCourseExportProvider extends InterfaceNativeGenerateRowData<ReportCanceledCourseData>{
	@NonNull
    ...
    private List<ReportCanceledCourseData> rawData;
    private ReportCanceledCourseData Total;

    @Override
    public DataResponse<ReportCanceledCourseData> getData() throws Exception {
        ...
        return DataResponse.<ReportCanceledCourseData>builder()
//            .datas(rawData)
		...
		return new BodyWriteHandler() {
			...
			public void writeBody(DataResponse data, SXSSFWorkbook workbook, Sheet sheet, AtomicInteger rowIndex) {
				...
				for (Map<String, Object> dataReport  : datas) {
					...
					wt.dateWrite( (Date) dataReport.get("request_cancel_date"), columnIndex.getAndIncrement()) //canceled_date
					  .stringWrite((String) dataReport.get("full_course_th"), columnIndex.getAndIncrement())
				      .doubleWrite((Long) dataReport.get("course_generation"), columnIndex.getAndIncrement())
    ...
    public HeaderHandler writeHeader() {
        return new HeaderHandler() {
            ...
            public void writeHeader(SXSSFRow headerRow, SXSSFWorkbook workbook) {
                ...
                // List<String> headers = new ArrayList<String>(Arrays.asList("No", "Course cancellation date", "Course", 
                // 		"Number of registrations","Number of students who must be refunded", "Status"));
                
                List<String> headers = new ArrayList<String>(Arrays.asList("ลำดับ", "วันที่ยกเลิกรอบคอร์ส", "รอบคอร์ส",
                        "รุ่น", "จำนวนผู้เรียนที่ต้องคืนเงิน", "จำนวนเงินที่ต้องคืน" ));
