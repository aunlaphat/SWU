...
@Builder
@Data
public class RequestApproveCourseProvider extends EmailProviderInterface implements ApiConstant
{
    ...
    public Map<String, Object> generateParam() {
        ...
        List<RequestApproveCourseData> datas = getData(statusId, courseId);
        ...
        param.put(DATE_REQUIRE, CommonUtils.formatDate(data.getCreateDate(), CommonUtils.formatDateDefault, CommonUtils.LOCALE_THAI));
        param.put(DATE_REQUIRE_EN, CommonUtils.formatDate(data.getCreateDate(), CommonUtils.formatDateDefault, CommonUtils.LOCALE_ENG));
        ...
        String json = asJsonString(CourseMain.builder().courseId(courseId).build());
    ...
    private List<RequestApproveCourseData> getData(Long statusId, Long courseId){
        String sql = """
            select cm.course_id, concat(mp.firstname_th , ' ', mp.lastname_th) as professor,
            concat(mp.firstname_en , ' ', mp.lastname_en) as professor_en,
            mp.email as professor_email, cm.course_code, cm.course_name_th , cm.course_name_en, cl.create_date, 
            mct.course_type_code, mct.course_type_name_th, mct.course_type_name_en, lc.name as teach_type, 
                    (SELECT string_agg(concat(ss.subject_code_th,' ',ss.subject_name_th), ', ')
                ...
                WHERE cm2.course_id = cm.course_id) as course_compare_th , (SELECT string_agg(concat(ss.subject_code_en,' ',ss.subject_name_en), ', ')
                    FROM course_matching cm2 
                join swu_subject ss on cm2.subject_swu_id = ss.subject_swu_id and cm2.curriculum_swu_id = ss.curriculum_swu_id
                WHERE cm2.course_id = cm.course_id) as course_compare_en, cm.create_by_id, au.ref_user_type, au.user_id,
            ...
            from course_main cm 
            join course_instructor ci on ci.course_id =cm.course_id and ci.instructor_main=true
            join mas_personal mp on ci.instructor_id = mp.personal_id 
            join course_log cl on cl.course_id = cm.course_id and cl.course_main_status = ?
            ...
            where au.ref_user_type ='30032001' and cm.course_id = ?
              limit 1  """;
        return jdbcTemplate.query(sql, BeanPropertyRowMapper.newInstance(RequestApproveCourseData.class), statusId, courseId);
    }
}
