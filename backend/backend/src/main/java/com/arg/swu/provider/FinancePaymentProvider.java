...
@Builder
@Slf4j
public class FinancePaymentProvider extends SignProvider{
    ...
    public void builderData() {
		...
		this.fileNameScr = data.getReceiptOriginalNonCaPath();
        if(moduleFile == null)
        {
    ...
    public void updateFileResult(byte[] file) throws Exception {
        ...
        UploadFileData rest = fss.saveByteArrayToStorage(file, this.fileNameDes, this.moduleFile);
		
        data.setReceiptOriginalCaPath(rest.getFullpath());  
